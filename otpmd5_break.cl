 ...
 * - Using the builtin rotate function in-fact makes the CPU code worse.  I'm assuing since
...
__kernel void otpmd5_break(__global unsigned int* input, __global unsigned int* output, const unsigned int rounds)
{
	size_t input_offset = get_global_id(0)*2;

	unsigned int X0 = input[input_offset];
	unsigned int X1 = input[input_offset+1];
	...
	for(int i = 0; i < rounds; i++)
	{
		...
		size_t output_offset = ((get_global_size(0) * i) + get_global_id(0)) * 2;
		output[output_offset] = X0;
		output[output_offset+1] = X1;
	}

	/* Allow us to just continue with the next lot without involving the host */
	input[input_offset] = X0;
	input[input_offset+1] = X1;
}
